import type { Metadata } from "next";
import { Suspense } from "react";
import { RegisterForm } from "@/components/auth";

export const metadata: Metadata = {
  title: "Create an Account | Telos Cart - Digital Storefront",
  description: "Join Telos Cart to track live shipments, save delivery addresses, and manage your orders.",
};

export default function RegisterPage() {
  return (
    <div className="min-h-[calc(100vh-14rem)] w-full">
      <Suspense fallback={<div className="min-h-[50vh] flex items-center justify-center"><div className="h-8 w-8 rounded-full border-2 border-emerald-500 border-t-transparent animate-spin" /></div>}>
        <RegisterForm />
      </Suspense>
    </div>
  );
}
