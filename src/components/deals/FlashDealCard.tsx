...
export function FlashDealCard({
  ...
  return (
    <div
      className={cn(
        "group relative flex flex-col h-full rounded-2xl sm:rounded-3xl bg-card text-card-foreground p-2 sm:p-3.5 border border-amber-500/15 hover:border-amber-500/15 transition-all duration-300 ease-out hover:-translate-y-1.5 shadow-[0_4px_16px_-4px_rgba(245,158,11,0.08)] dark:shadow-[0_4px_20px_-4px_rgba(0,0,0,0.5)] hover:shadow-[0_16px_36px_-8px_rgba(245,158,11,0.22)]",
        className
      )}
    >
      {/* Top Media Container */}
      <div className="relative aspect-square w-full overflow-hidden rounded-lg sm:rounded-2xl bg-muted/40 dark:bg-zinc-900/60">
        ...
        <div className="absolute top-1.5 left-1.5 sm:top-2.5 sm:left-2.5 flex items-center gap-0.5 sm:gap-1 rounded-full bg-gradient-to-r from-amber-500 to-orange-500 px-1.5 sm:px-2.5 py-0.5 text-[9px] sm:text-[11px] font-black text-zinc-950 shadow-xs sm:shadow-md">
          <Zap className="h-2.5 w-2.5 sm:h-3 sm:w-3 fill-zinc-950" />
        ...
        {!isAdmin && (
          <button
            ...
            className={cn(
              "absolute top-1.5 right-1.5 sm:top-2.5 sm:right-2.5 flex h-6.5 w-6.5 sm:h-8 sm:w-8 items-center justify-center rounded-full bg-background/85 backdrop-blur-md shadow-2xs transition-all duration-200 hover:scale-110 active:scale-95 cursor-pointer",
              isWishlisted
      ...
      <div className="flex flex-1 flex-col pt-2 sm:pt-3 pb-0.5 sm:pb-1">
        {/* Brand & Genuine Tag */}
        <div className="flex items-center justify-between text-[9px] sm:text-[11px] text-muted-foreground">
          <span className="font-extrabold uppercase tracking-wider text-amber-600 dark:text-amber-400 truncate max-w-[90px] sm:max-w-none">
            {product.brand}
          </span>
          <span className="hidden xs:flex items-center gap-0.5 text-emerald-600 dark:text-emerald-400 font-semibold text-[9px] sm:text-[11px]">
        ...
        {/* Product Title */}
        <div className="mt-0.5 sm:mt-1 h-7 sm:h-10 flex items-start">
          <Link
            href={productUrl}
            className="font-bold text-[11px] sm:text-sm text-foreground line-clamp-2 hover:text-amber-500 transition-colors leading-tight sm:leading-snug"
          >
            {product.name}
          </Link>
        </div>
        ...
        {/* Rating */}
        <div className="mt-0.5 sm:mt-1 flex items-center gap-1 text-[10px] sm:text-xs">
          <div className="flex items-center text-amber-500 shrink-0">
            <Star className="h-2.5 w-2.5 sm:h-3 sm:w-3 fill-amber-400 text-amber-400" />
            <span className="ml-0.5 sm:ml-1 font-bold text-foreground text-[10px] sm:text-[11px]">
              {product.rating.toFixed(1)}
            </span>
          </div>
          <span className="hidden sm:inline text-[10px] text-muted-foreground">({product.reviewCount})</span>
        </div>

        {/* Price & Savings */}
        <div className="mt-1.5 sm:mt-2.5 flex items-baseline justify-between gap-1">
          <div className="flex items-baseline gap-1 sm:gap-1.5 flex-wrap">
            <span className="text-xs sm:text-base font-black text-foreground tracking-tight">
          ...
          {savingsAmount > 0 && (
            <span className="hidden sm:inline-block rounded-md bg-emerald-500/10 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 font-bold text-[9px] sm:text-[10px] px-1.5 py-0.5 whitespace-nowrap">
        ...
        <div className="mt-1.5 sm:mt-3">
          <div className="flex items-center justify-between text-[9px] sm:text-[11px] mb-0.5 sm:mb-1 font-semibold">
            <span className="flex items-center gap-0.5 sm:gap-1 text-orange-600 dark:text-orange-400 font-bold">
              <Flame className="h-2.5 w-2.5 sm:h-3 sm:w-3 fill-orange-500 text-orange-500 animate-pulse" />
              <span>{claimed}%</span>
            </span>
            <span className="text-muted-foreground text-[9px] sm:text-[10px]">
              <strong className="text-foreground font-black">{left}</strong> left
            </span>
          </div>
          <div
            role="progressbar"
            ...
            className="relative h-1.5 sm:h-2 w-full overflow-hidden rounded-full bg-amber-500/15 dark:bg-zinc-800"
          >
            <div
        ...
        <div className="mt-2 sm:mt-3.5">
          <button
            ...
            className={cn(
              "w-full flex h-7 sm:h-9 items-center justify-center gap-1 sm:gap-1.5 rounded-lg sm:rounded-2xl px-2 sm:px-3 text-[10px] sm:text-xs font-black transition-all duration-200 active:scale-95 cursor-pointer shadow-xs",
              isAdmin
          ...
          >
            {isAdmin ? (
              ...
              <>
                <Flame className="h-3 w-3 sm:h-3.5 sm:w-3.5 fill-zinc-950" />
